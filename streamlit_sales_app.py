...
def research_company(company_name, domain, openai_key):
    ...
    if not openai_key:
        st.warning(f"No OpenAI API key - skipping research for {company_name}")
        return {"research": f"Research unavailable for {company_name}. Consider adding OpenAI API key for detailed company insights."}
    
    try:
        st.write(f"**Debug: Researching {company_name}...**")
        ...
        st.write(f"**Debug: Research completed for {company_name}**")
        
        return {"research": research_result}
        
    except Exception as e:
        st.error(f"**Research failed for {company_name}: {str(e)}**")
        st.write(f"**Debug: Research error type: {type(e).__name__}**")
        
        return {"research": f"Research unavailable for {company_name} due to API error. {company_name} appears to be a potential customer in their industry."}
        

def generate_email(prospect_data, research_data, openai_key):
    """Generate personalized email with better error handling"""
    if not openai_key:
        st.warning("No OpenAI API key provided - generating basic email template")
        return {
            "subject": f"Partnership opportunity with {prospect_data.get('company_name', 'your company')}",
            "body": f"""Hi there,

I hope this message finds you well. I've been researching companies in the {prospect_data.get('industry', 'your industry')} space and {prospect_data.get('company_name', 'your company')} caught my attention.

We help companies like yours streamline their operations and drive growth. I'd love to explore how we might be able to support your goals.
...
[Your Name]"""
    ...
    try:
        st.write(f"**Debug: Generating email for {prospect_data['company_name']}**")
        
        client = openai.OpenAI(api_key=openai_key)
        
        research_text = research_data.get('research', 'No specific research available')
        ...
        2. Reference specific company insights if available
        ...
        st.write(f"**Debug: Sending prompt to OpenAI...**")
        ...
        st.write(f"**Debug: Received response from OpenAI**")
        
        email_content = response.choices[0].message.content
        st.write(f"**Debug: Raw email content: {email_content[:100]}...**")
        ...
        st.write(f"**Debug: Parsed subject: {subject}**")
        st.write(f"**Debug: Email generation successful**")
        
        return {"subject": subject, "body": body}
        
    except Exception as e:
        st.error(f"**Email generation failed for {prospect_data['company_name']}: {str(e)}**")
...
I hope this message finds you well. I noticed {prospect_data.get('company_name', 'your company')} and thought we might be able to help with your business goals.

We work with companies in {prospect_data.get('industry', 'your industry')} to drive growth and improve efficiency.
...
Note: This is a fallback email due to API generation issues."""
        }
    ...
    with tab1:
        ...
        with col1:
            ...
            crm_file = st.file_uploader(
                ...
                type=['csv'],
                key="crm_upload",
                help="Expected columns: company_name, won (required), industry, company_size, deal_value, time_to_close, location (optional)"
            )
        ...
        if not st.session_state.model_trained:
            ...
            if st.button("Process Leads", type="primary"):
                ...
                for idx, (_, lead) in enumerate(leads_df.iterrows()):
                    ...
                    if score >= min_score:
                        # Research company
                        st.write(f"**Debug: Starting research for {lead['company_name']}...**")
                        ...
                        # Generate email
                        st.write(f"**Debug: Starting email generation for {lead['company_name']}...**")
